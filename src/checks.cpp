...
CheckResult check_git_installed() {
    ...
    return {result == 0, result == 0 ? "Git is installed" : "Git is not installed or not in PATH"};
}
...
CheckResult check_node_installed() {
    ...
    if (sscanf(version.c_str(), "v%d", &major_version) == 1) {
        if (major_version >= 16) {
            return {true, "Node.js v" + std::to_string(major_version) + " is installed"};
        }
        return {false, "Node.js version too old (v" + std::to_string(major_version) + "), need v16+"};
    }
    
    return {true, "Node.js is installed (version check inconclusive)"};
}
...
CheckResult check_folder_structure() {
    ...
    return {false, "Missing required files: " + missing};
}
...
CheckResult check_api_tokens() {
    ...
    if (protocol_end == std::string::npos) {
        return {false, "Invalid API URL format (missing protocol)"};
    }
    ...
    if (port == 443) {
        ...
        if (!ctx) {
            return {false, "SSL context creation failed"};
        }
    }
    ...
    if (bytes_sent <= 0) {
        ...
        close(sock);
        if (ctx) SSL_CTX_free(ctx);
        return {false, "Failed to send heartbeat"};
    }
    ...
    if (response.find("HTTP/1.1 20") != std::string::npos) {  
        return {true, "Heartbeat sent successfully, hackatime is working!"};
    }
